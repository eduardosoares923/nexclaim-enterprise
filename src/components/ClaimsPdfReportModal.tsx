              ...
              <div key={i} className="border border-slate-200 rounded-lg p-3 break-inside-avoid bg-white">
                <div className="flex items-center justify-between border-b border-slate-100 pb-2 mb-2">
                  <div>
                    ...
                    {c.protocol && <span className="text-slate-400 text-[10px] ml-2 font-mono">Protocolo: {c.protocol}</span>}
                  </div>
                  <div className="flex items-center gap-2">
                    {c.date && <span className="text-slate-500 text-[10px] font-medium">{c.date}</span>}
                    {c.status && (
