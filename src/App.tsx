...
export const App: React.FC = () => {
  ...
  // Template State (Firestore)
  const { data: templatesSalvos = [], isLoading: loadingTemplates } = useTemplates(loginConfirmado);
  const createTemplateMutation = useCreateTemplate();
  const updateTemplateMutation = useUpdateTemplate();

  // Enquanto não houver nenhum modelo salvo no banco, usa os modelos oficiais do código
  const templates = templatesSalvos.length > 0 ? templatesSalvos : MODELOS_INICIAIS;
  ...
  const handleSaveTemplate = (updatedTemplate: DocumentTemplate) => {
    const { id, ...dados } = updatedTemplate;
    const jaExisteNoBanco = templatesSalvos.some((t) => t.id === id);
