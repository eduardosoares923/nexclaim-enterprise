...
window.Header = function Header({ currentUser, setCurrentUserRole, onOpenSearch, onOpenNewClaimModal }) {
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center bg-slate-100 p-1 rounded-lg border border-slate-200 text-xs">
          ...
          <select 
            ...
            class="bg-white border border-slate-200 rounded text-xs font-semibold px-2 py-1 text-slate-700 focus:outline-none cursor-pointer"
          >
            <option value="ADMINISTRADOR">ADMINISTRADOR (Acesso Total)</option>
            <option value="GESTOR">GESTOR (Gestão Operacional)</option>
            <option value="OPERADOR">OPERADOR (Cadastro & Atualizações)</option>
        ...
        <div class="relative">
          <button 
            ...
            class="w-9 h-9 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-600 flex items-center justify-center relative transition-colors"
          >
          ...
          {showNotifDropdown && (
            <div class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-50 animate-fade-in">
              <div class="px-4 py-2 border-b border-slate-100 flex items-center justify-between">
                <h4 class="text-xs font-bold text-slate-800">Central de Notificações</h4>
                <span class="text-[10px] text-blue-600 font-semibold cursor-pointer">Limpar</span>
              </div>
        ...
        {/* Quick Add Claim Button */}
        <button 
          onClick={onOpenNewClaimModal}
          class="btn btn-primary text-xs shadow-sm bg-blue-600 hover:bg-blue-700 text-white font-semibold px-3 py-2 rounded-lg flex items-center gap-1.5"
        >
